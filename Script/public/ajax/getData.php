  ...
  if( isset( $_GET ) && $_SERVER['REQUEST_METHOD'] == "GET" ) {
	...
	if( $countFavs != 0 ) {
		for ( $i = 0; $i < $countFavs; ++$i ) { 
			$favoriteArr[] = '<a href="'.URL_BASE.$getFavorites[$i]['username'] .'" class="myicon-right showTooltip openModal" data-id="'.$getFavorites[$i]['id'].'" title="@'.$getFavorites[$i]['username'].'" data-toggle="tooltip" data-placement="top"> <img src="'.URL_BASE.'thumb/20-20-public/avatar/'.$getFavorites[$i]['avatar'] .'" class="img-rounded" width="20"> </a>';
		}
	}//<--- 
	...
	echo json_encode( array ( 
			...
			<li class="list-breadc">'.date('d/m/Y', strtotime( $getMedia[0]['date'] ) ).'</li>
